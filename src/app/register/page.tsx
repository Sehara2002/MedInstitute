...
export default function RegisterPage() {
  ...
  const [amountEditable, setAmountEditable] = useState(false); // only true for "On Request" items
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      if (p !== null) {
        ...
        setAmountEditable(true); // genuinely no fixed price — needs manual entry/review
      }
    }
  }, [idParam]);
    ...
    try {
      const res = await fetch("/api/registration", {
        ...
        body: JSON.stringify({
          ...
          phone,
          courseId: idParam,
          // only sent as a fallback for genuine "On Request" items;
          // ignored server-side whenever courseId resolves to a known price
          course,
          amount,
        }),
      });
